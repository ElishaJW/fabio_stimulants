...
landuse_cf_raw <- fread("X:/Eli/data/cf/land use/CF_domain_country.csv")
water_cf_raw <- read.xlsx("X:/Eli/data/cf/water consumption/water.xlsx", sheet="Country_CF")
N_cf_raw <- read.xlsx("X:/Eli/data/cf/freshwater eutrophication/CFs_freshwater_eutrophication/global_species_loss/N/Country_CF_N.xlsx")
P_cf_raw <- read.xlsx("X:/Eli/data/cf/freshwater eutrophication/CFs_freshwater_eutrophication/global_species_loss/P/Country_CF_P.xlsx")
#cc_cf_raw <- read.xlsx("X:/Eli/data/cf/climate change/GLAM_template_EQ_Climate_Change_FW_TR_MA.xlsx", sheet="lciamethods_CF_GLAM")
cc_cf_raw <- read.xlsx("X:/Eli/data/cf/climate change/EQ_ClimateChange_Terrestrial_gloPDF_Country_Aggregated_2025-12-09.xlsx", sheet="NaturalEarth")
...
############### FIRST SUM GHG STRESSORS IN E MATRIX ############################
...
# Define a list of GHG stressor names to sum contained in original stressor_list
stressor_list <- colnames(stressor_list)[2:ncol(stressor_list)]
stressors_to_sum <- c("CH4", "CO2", "N2O")

# Loop over each year, adding all CO2 stressors together, CH4 together, and N20
for (year in years) {
  
  # Get the current stressor matrix
  E_name <- paste0("E_", year)
  E_year <- readRDS(paste0("./data/E/E_full_", year, ".rds"))
...
# Need to run this for loop in order to process cfs for other parts of this code


for (year in years){
  str_fp <- readRDS(paste0("./data/stressor footprints/str_footprint_",year,".rds"))
  ...
  landuse_cf <- landuse_cf_raw[grepl("Intense", landuse_cf_raw$habitat), ]
  ...
  # Drop unnecessary columns
  water_cf <- water_cf_raw[, c("ISO3CD", "CF_GLOB_A_m", "Contains.outliers")]
  # Rename column 'iso3cd' to 'iso3c'
  ...
  # Preference for countries with no outliers
  water_cf <- water_cf %>%
    group_by(iso3c) %>%
    filter(Outliers == FALSE | !any(Outliers == FALSE)) %>%
    slice(1)  # Ensure only one row is kept if both TRUE and FALSE exist in outliers column
  ...
  for (nutrient in c("N", "P")){
    ...
    # Select CF
    nutrient_cf <- get(paste0(nutrient, "_cf_raw"))
    # Reformat columns
  ...
  # bd_rows["CO2_bd", ] <- as.numeric(E_year["CO2", , drop = FALSE]) * cc_cf$CF[cc_cf$FLOW_name == "carbon dioxide"]
  # bd_rows["CH4_bd", ] <- as.numeric(E_year["CH4", , drop = FALSE]) * cc_cf$CF[cc_cf$FLOW_name == "methane"]
  # bd_rows["N2O_bd", ] <- as.numeric(E_year["N2O", , drop = FALSE]) * cc_cf$CF[cc_cf$FLOW_name == "nitrous oxide"]
  # 
  
  
  # SUM THE MATRICES TOGETHER-----------------------------------------------------
  # Simple sum of dataframes by realm, first changing NA to 0 if necessary
  ...
  # Average freshwater and terrestrial realms to get total
  bd_fp_total <- (bd_fp_fw + bd_fp_landuse) / 2
  
  rownames(bd_fp_total) <-   rownames(bd_fp_water)
  
  # Save!
  saveRDS(bd_fp_total, file=paste0("./data/biodiversity footprints/bd_fp_total_", year, ".rds"))
  write.csv(
    as.matrix(bd_fp_total),
    paste0("./data/biodiversity footprints/bd_fp_total_", year, ".csv"),
    row.names = TRUE,
    col.names = TRUE
  )
}
...
for (year in years) {
  ...
  # Save
  saveRDS(E_new, file=paste0("./data/E/E_bd_", year, ".rds"))
  write.csv(
    as.matrix(E_new),
    paste0("./data/E/E_bd_", year, ".csv"),
    row.names = TRUE,
    col.names = TRUE
  )
}
